...
- **Componentes de app sobre Spectrum**: `finap-input` → `sp-textfield`/`sp-number-field` (con `textarea` y `date` como controles nativos estilizados con tokens), `finap-select` → `sp-picker`, `finap-modal` → `sp-dialog` (underlay incluido), `finap-progress` → `sp-progress-bar`, `finap-chip` → `sp-tag` (con estado seleccionado propio), `finap-avatar` → `sp-avatar`. `finap-stat-card` y `finap-list-item` se recomponen con estilos/tokens Spectrum manteniendo su API.
...
- **Tests**: unitarios de los componentes migrados se actualizan **en esta fase** (aseguran que la suite sigue verde); el E2E se ajusta igualmente aquí (selectores de picker/diálogos), y la fase 3 (`ui-spectrum-verification`) hace el repaso final de PWA/bundle/limpieza.
