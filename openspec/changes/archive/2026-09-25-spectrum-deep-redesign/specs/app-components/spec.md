...
**Reason**: Sustituido por `sp-dialog-wrapper`, el componente de diálogo modal de Spectrum (overlay, foco y cierre incluidos).

**Migration**: Usar `<sp-dialog-wrapper>` directamente en las vistas (título, contenido y acciones por slots), conservando los eventos de negocio (confirmar/cancelar) en la vista.

### Requirement: finap-stat-card

**Reason**: Sustituido por la composición directa de `sp-card` + `sp-badge` en las vistas.

**Migration**: Componer la métrica con `<sp-card>` y mostrar la variación con `<sp-badge>` en la vista correspondiente.

### Requirement: finap-list-item

**Reason**: Sustituido por `sp-table` (listados) y filas compuestas con primitivas de Spectrum.

**Migration**: Usar `<sp-table>` para listados y composiciones de fila con tokens de Spectrum en las vistas que lo usaban.

### Requirement: finap-progress

**Reason**: Sustituido por `sp-meter`/`sp-progress-bar` de Spectrum.

**Migration**: Usar `<sp-meter>` o `<sp-progress-bar>` directamente con el valor calculado en la vista.

### Requirement: finap-chip

**Reason**: Sustituido por `sp-tag` de Spectrum.

**Migration**: Usar `<sp-tag>` (con `selected`/rol de botón cuando aplique) directamente en las vistas.

### Requirement: finap-avatar

**Reason**: Sustituido por `sp-avatar` de Spectrum.

**Migration**: Usar `<sp-avatar>` (imagen o `label`) directamente en las vistas.
